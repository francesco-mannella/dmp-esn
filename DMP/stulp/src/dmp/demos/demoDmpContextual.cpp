 ...
 */
bool getDemoTrajectories(VectorXd& task_parameters_demos, vector<Trajectory>& trajectories, 
        string directory_trajectories="", bool test=false);
...
int main(int n_args, char** args)
{
  ...
  vector<Trajectory> test_trajectories;
  VectorXd task_parameters_demos;
  VectorXd task_parameters_test_demos;
  ...
  getDemoTrajectories(task_parameters_test_demos, test_trajectories, directory_trajectories, true);
  
  int n_demos = trajectories.size();
  int n_tests = test_trajectories.size();
  ...
  VectorXd task_parameters_repros = VectorXd::LinSpaced(n_repros,task_parameters_demos.minCoeff(),task_parameters_demos.maxCoeff()); 
  vector<Trajectory> trajs_reproduced(n_repros);
  ...
  if (!directory.empty())
  {
    ...
    cout << "Saving test trajectories to " << directory << endl;
    for (int i_test=0; i_test<n_tests; i_test++)
    {
      stringstream stream;
      stream << "test" << setw(2) << setfill('0') << i_test << ".txt";
      test_trajectories[i_test].saveToFile(directory,stream.str(),overwrite);
    }
...
bool getDemoTrajectories(VectorXd& task_parameters_demos, vector<Trajectory>& trajectories,
        string directory_trajectories, bool test)
{
  ...
  if (directory_trajectories.empty())
  {
    ...
    if(not test)
        ...
        task_parameters_demos = VectorXd::LinSpaced(n_demos,0.275,0.725);
