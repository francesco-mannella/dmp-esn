...
def plot_trajectories(Y, L, A, ax, color):
    ...
    for y,i in zip(Y,range(len(Y))):
        y[0,:] = y[0,:] + A[i]*4.0 
        y[1,:] = y[1,:] + L[i]*4.0
        ax.plot(*y, color=color, lw=3)
        ax.plot(np.array([0.5,0.5])+L[i]*4.0, 0.5+np.array([-.2,.2])+A[i]*4.0, color="black", lw=.5)
        ax.plot(0.5+np.array([-.2,.2])+L[i]*4.0,  np.array([0.5,0.5]) + A[i]*4.0 , color="black", lw=.5)
    
    ax.set_xlim([0,5]) 
    ax.set_ylim([0,5]) 
    ax.set_xticks(0.5+L*4) 
    ax.set_xticklabels(L, rotation='vertical') 
    ax.set_yticks(0.5+A*4) 
